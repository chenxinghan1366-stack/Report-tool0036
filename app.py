...
def save_companies(companies):
    ...
    for comp in companies:
        if comp.get('province') and comp['province'].strip() and comp['province'] != '未知':
...
def save_rules(rules):
    ...
    for r in rules:
        if r.get('province') and r['province'].strip() and r['province'] != '未知':
...
CITY_TO_PROVINCE = {}
for province, cities in CHINA_REGION_MAP.items():
    for city in cities:
        city_norm = normalize_name(city)
        CITY_TO_PROVINCE[city_norm] = province
...
def parse_companies_from_sheet(file, sheet_name, user_id="default"):
    try:
        ...
        for _, row in df.iterrows():
            ...
            if province and province.strip() and province != '未知':
                companies.append({
    ...
    if province:
        ...
        for r in rules:
            if normalize_name(r.get('province', '')) == norm_prov:
                ...
                return fallback
    mapped_province = get_province_for_city(city)
    if mapped_province:
        province = mapped_province
    else:
        return None
    ...
    if uploaded_files:
        with st.spinner("正在解析Excel..."):
            ...
            if not companies and columns:
                ...
                if city_col and company_col:
                    ...
                    for _, row in df.iterrows():
                        ...
                        if city and company:
                            norm_city = normalize_name(city)
                            province = get_province_for_city(norm_city)
                            if not province:
                                rules = load_rules()
                                for r in rules:
                                    if normalize_name(r['city']) == norm_city:
            ...
            if companies:
                ...
                if unmapped:
                    st.warning(f"⚠️ 以下城市未识别到省份：{', '.join(unmapped)}，已跳过")
...
if not companies:
    ...
    unmapped_companies = []
    for c in companies:
        if c['province'] and c['province'].strip() and c['province'] != '未知':
    ...
    if unmapped_companies:
        st.warning(f"⚠️ 有 {len(unmapped_companies)} 家公司的省份无法识别，已跳过。请检查数据中'省份'列是否正确。")
        with st.expander("查看无法识别的公司"):
            for uc in unmapped_companies:
                st.write(f"- {uc['company_name']}（城市：{uc['city']}）")
